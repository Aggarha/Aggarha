...
const bookingInclude = {
  ...
  deal: { select: { reviews: { select: { reviewerId: true } } } }
} as const;
...
function serializeBooking(
  booking: {
    ...
    deal: { reviews: { reviewerId: string }[] } | null;
  },
  viewerId: string
) {
  return {
    ...
    alreadyReviewed: booking.deal?.reviews.some((review) => review.reviewerId === viewerId) ?? false
  };
}
...
export default async function BookingsPage() {
  ...
  return (
    <div className="mx-auto w-full max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
      <BookingStatusDemo
        lang={locale}
        renterBookings={renterBookings.map((booking) => serializeBooking(booking, session.userId))}
        ownerBookings={ownerBookings.map((booking) => serializeBooking(booking, session.userId))}
      />
    </div>
  );
}
